...
import { ADD_ORDER, GET_ONE_ORDER, GET_ALL_ORDERS, GET_ALL_CLIENT_ORDERS, GET_ALL_DRIVER_ORDERS, CHANGE_STATUS } from "../types/orderTypes";
...
export const getAllActiveOrdersFromServer = () =>
  async (dispatch) => {
    const response = await axios("http://localhost:3032/activeOrders")
...
export const getClientOrdersFromServer = () =>
  async (dispatch) => {
    const response = await axios("http://localhost:3032/clientOrders")
...
export const getDriverOrdersFromServer = () =>
  async (dispatch) => {
    const response = await axios("http://localhost:3032/driverOrders")
